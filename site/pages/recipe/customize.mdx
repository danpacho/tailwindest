...
<Callout type="info">
    You can define **custom accessor keyword** via `Tailwindest`'s third generic
    type.
</Callout>
...
```ts filename="tw.ts" /screens/ /my1/ /my2/ /my3/
type TailwindCustom = Tailwindest<{
    screens: "my1" | "my2" | "my3"
}>
...
```ts filename="tw.ts" /color/ /primary/ /secondary/
type TailwindCustom = Tailwindest<{
...
```ts filename="tw.ts" /opacity/ /11/ /12/ /13/
type TailwindCustom = Tailwindest<{
    opacity: "11" | "12" | "13"
}>
...
```ts filename="tw.ts" /spacing/ /0.25/ /0.75/ /1.25/
type TailwindCustom = Tailwindest<{
...
```ts filename="tw.ts" /fontFamily/ /display/ /body/
type TailwindCustom = Tailwindest<
    {},
    {
