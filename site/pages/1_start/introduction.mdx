...
Unlike `twin` and other css in js frameworks, `tailwindest` is just **`className` string generator function** like `clsx`, but optimized only for `tailwindcss`.
And the fact that `tailwindest` is a `className` string generator gives it the following advantages.

1. **Tiny bundle size, `638B`**
...
`variants` are **combinations of rotary switches** - think of the **microwave at home**. You'll notice that it has a few rotary switches, such as a **timer** and an **intensity**.
...
If you apply `tailwindest` at right point, you can design a much more robust product with `tailwind`!

## Real world adaptation

1. Short: `inline-class`
2. Complex & Reusable: `tailwindest`
...
1. Use `tailwind` to **create a design system**
